...
export function ChartsWrapper({
  ...
  const chartColors = [
    ...
    "var(--teal-500)", // Teal
    "var(--ochre-500)", // Ochre
    "var(--stone-400)", // Stone (4th)
  ]

  return (
    <div
      className={cn(
        "rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--stone-0)] p-5 flex flex-col gap-4 dark:bg-[var(--stone-900)] dark:border-[var(--stone-800)] text-start shadow-sm select-none",
        className
      )}
    >
      {/* Title & Legend Header */}
      ...
      {/* Main Plot Area */}
      <div className="h-[200px] w-full flex items-end justify-between gap-4 border-b border-l border-[var(--border-default)] pb-2 ps-2 relative dark:border-[var(--stone-800)]">
        {type === "bar" && (
          <div className="w-full h-full flex items-end justify-around pt-4">
            {labels.map((lbl, lIdx) => (
              <div key={lIdx} className="flex flex-col items-center gap-1 flex-1 max-w-[60px]">
                ...
                <span className="text-[10px] text-[var(--stone-500)] mt-1 truncate max-w-full">
                  {lbl}
                </span>
              </div>
            ))}
        ...
        {type === "donut" && (
          <div className="absolute inset-0 flex items-center justify-center pt-2">
            <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 42 42">
              <circle cx="21" cy="21" r="15.915" fill="transparent" stroke="var(--stone-100)" strokeWidth="4" />
              {/* Pie slices */}
              <circle
                cx="21"
                cy="21"
            ...
            <div className="absolute flex flex-col text-center leading-none select-none">
              ...
              <span className="text-[10px] text-[var(--stone-500)] mt-1">إجمالي الحالات</span>
            </div>
          </div>
        )}
      </div>
      ...
      {type === "area" && labels.length > 0 && (
        <div className="flex justify-between px-2 text-[10px] text-[var(--stone-500)]">
          {labels.map((lbl, idx) => (
